...
export const updateSetting = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ message: 'Internal server error while updating setting.' })
  }
}
...
export const updateMultipleSettings = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ message: 'Internal server error while updating multiple settings.' })
  }
}
...
export const createSetting = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ message: 'Internal server error while creating setting.' })
  }
}
...
export const updateBranch = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // --- TASK 79 FIX: STRICT DELIVERY RADIUS LIMIT & VALIDATION ---
    if (deliveryRadiusKm !== undefined && deliveryRadiusKm !== null) {
      const radius = parseFloat(deliveryRadiusKm)
      if (isNaN(radius) || radius <= 0) {
        res.status(400).json({ message: 'Delivery radius must be a positive number greater than 0' })
        return
      }
      if (radius > 100) {
        res.status(400).json({ message: 'Delivery radius cannot exceed the 100 km limit.' })
        return
      }
    }
    ...
    console.error('CRITICAL ERROR in updateBranch:', error)
    // Server is now shielded. It returns 500 cleanly instead of committing suicide!
    res.status(500).json({ message: 'Server encountered an error saving branch details. Please check your inputs.' })
  }
}

export const toggleRestaurantOpen = async (req: Request, res: Response): Promise<void> => {
  try {
    const branch = await prisma.branch.findUnique({ where: { id: 'main-branch' } })
    ...
    console.error('Error toggling restaurant state:', error)
    res.status(500).json({ message: 'Internal server error while toggling restaurant state.' })
  }
}