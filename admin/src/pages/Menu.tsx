...
function CategoriesTab() {
  ...
  const handleToggle = async (cat: Category) => {
    await api.patch(`/api/admin/menu/categories/${cat.id}/availability`)
    fetchCategories()
  }
    ...
    <div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Category' : 'New Category'}>
        <div className="space-y-3">
          ...
          <button
            onClick={handleSave}
            className="w-full bg-brand-600 text-white py-2.5 rounded-lg font-medium mt-2"
          >
...
function ItemsTab() {
  ...
  const handleSave = async () => {
    if (!form.name.trim() || !form.basePrice) return alert('Name and price are required')
...
function PackagesTab() {
  ...
  const handleSave = async () => {
    if (!form.name.trim() || !form.totalPrice) return alert('Name and price are required')
    if (packageItems.length === 0) return alert('Add at least one item')
    ...
    if (editing) {
      ...
      await api.post('/api/admin/menu/packages', payload)
    }
    setModalOpen(false)
    fetchPackages()
  }
  ...
  const handleToggle = async (pkg: Package) => {
    await api.patch(`/api/admin/menu/packages/${pkg.id}/availability`)
    fetchPackages()
  }
            ...
            <div className="space-y-2">
              {packageItems.map((pi, i) => (
                <div key={i} className="flex gap-2">
                  ...
                  />
                  <button onClick={() => removeItemRow(i)} className="text-red-500 px-2">&times;</button>
                </div>
              ))}
            </div>
          </div>
