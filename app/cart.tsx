...
const CartItemCard = ({ item, isSelected, onToggle, onIncrease, onDecrease, onRemove, colors, isDark }: any) => {
  ...
  return (
    <View style={[styles.cartItem, { backgroundColor: isLocked ? (isDark ? '#1A1A1A' : '#E0E0E0') : colors.surface }]}>
      {isLocked && (
      ...
      <TouchableOpacity onPress={() => isLocked ? toggleExpand() : onToggle(item.id)} activeOpacity={isLocked ? 1 : 0.8} style={styles.imageWrapper}>
        <Image source={{ uri: item.image }} style={[styles.itemImage, isLocked && { opacity: 0.3 }]} resizeMode="cover" />
        {!isLocked && (
          <Animated.View style={[styles.selectedOverlay, { opacity: scaleAnim }]}>
            <Animated.View style={{ transform: [{ scale: scaleAnim }] }}><Ionicons name="checkmark-circle" size={36} color="#FFF" /></Animated.View>
          </Animated.View>
        )}
      </TouchableOpacity>
      ...
      <View style={[styles.detailsWrapper, isLocked && { opacity: 0.6 }]}>
        <View style={styles.topRow}>
          <Text style={[styles.itemName, { color: isLocked ? colors.textMuted : colors.text }]} numberOfLines={1}>{item.category || item.name}</Text>
          <TouchableOpacity onPress={() => onRemove(item.id)} style={styles.deleteBtn}><Ionicons name="close" size={20} color={isLocked ? colors.text : colors.textMuted} /></TouchableOpacity>
        </View>

        <View style={styles.itemDetails}>
          <Text style={[styles.itemContents, { color: colors.textMuted }]} numberOfLines={1}>
            {item.subItems && item.subItems.length > 0 ? item.subItems.map((sub: any) => sub.name).join(' | ') : (item.contents || '')}
          </Text>

          {isLocked && isExpanded && (
            <View style={styles.expandedMissingBox}>
              {unavailableSubItems.map((sub: any, idx: number) => (
                <Text key={idx} style={styles.missingItemText} numberOfLines={1}>• {sub.name} (Sold Out)</Text>
              ))}
            </View>
          )}
          
          <View style={styles.priceAndActionRow}>
            <Text style={[styles.itemPrice, { color: isLocked ? colors.textMuted : Colors.primary }]}>₦{item.price.toLocaleString()}</Text>

            <View style={styles.rightActions}>
              <TouchableOpacity style={[styles.editBtn, { backgroundColor: isLocked ? '#FFEBEE' : (isDark ? 'rgba(255,255,255,0.1)' : '#F0F0F0') }]} onPress={() => router.push('/menu')}>
                <Text style={[styles.editBtnText, { color: isLocked ? '#D32F2F' : colors.text }]}>Edit</Text>
              </TouchableOpacity>
              
              <View style={[styles.quantityBox, { backgroundColor: isDark ? 'rgba(255,255,255,0.05)' : '#F5F5F5' }, isLocked && { opacity: 0.3 }]}>
                <TouchableOpacity onPress={() => !isLocked && onDecrease(item.id)} style={styles.qtyBtn} disabled={isLocked}><Ionicons name="remove" size={16} color={colors.text} /></TouchableOpacity>
                <Text style={[styles.qtyText, { color: colors.text }]}>{item.quantity}</Text>
                <TouchableOpacity onPress={() => !isLocked && onIncrease(item.id)} style={styles.qtyBtn} disabled={isLocked}><Ionicons name="add" size={16} color={colors.text} /></TouchableOpacity>
              </View>
            </View>
          </View>
        </View>
...
export default function CartScreen() {
  ...
  const subtotal = selectedItemsList.reduce((sum: number, item: any) => sum + (item.price * item.quantity), 0);
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {cartItems.length > 0 ? (
        <>
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
            ...
            <View style={styles.itemsContainer}>
              {cartItems.map((item: any) => (
                <CartItemCard key={item.id} item={item} isSelected={selectedIds.includes(item.id)} onToggle={(id: string) => setSelectedIds(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id])} onIncrease={increaseQuantity} onDecrease={decreaseQuantity} onRemove={handleRemove} colors={colors} isDark={isDark} />
              ))}
            </View>
          </ScrollView>
          <View style={[styles.stickyFooter, { paddingBottom: insets.bottom + 20, backgroundColor: isDark ? colors.surface : '#FFF', borderTopColor: colors.border }]}>
            <View style={styles.summaryContainer}>
              ...
              <View style={styles.summaryRow}><Text style={[styles.summaryLabel, { color: colors.textMuted }]}>Order</Text><Text style={[styles.summaryValue, { color: colors.text }]}>₦{subtotal.toLocaleString()}</Text></View>
              <View style={styles.summaryRow}><Text style={[styles.summaryLabel, { color: colors.textMuted }]}>Delivery Fee</Text><Text style={[styles.summaryValue, { color: colors.text }]}>₦{(subtotal > 0 ? 500 : 0).toLocaleString()}</Text></View>
              <View style={styles.summaryRow}><Text style={[styles.totalLabel, { color: colors.text }]}>Total Order</Text><Text style={[styles.totalValue, { color: Colors.primary }]}>₦{total.toLocaleString()}</Text></View>
              <TouchableOpacity style={[styles.checkoutBtn, selectedIds.length === 0 && { opacity: 0.5, backgroundColor: '#999' }]} disabled={selectedIds.length === 0} activeOpacity={0.8}><Text style={styles.checkoutText}>Order Now</Text></TouchableOpacity>
            </View>
          </View>
        </>
      ) : (
        <View style={styles.emptyContainer}>
          ...
          <TouchableOpacity style={styles.browseBtn} onPress={() => router.push('/menu')}><Text style={styles.browseBtnText}>Browse Menu</Text></TouchableOpacity>
        </View>
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  cartItem: { flexDirection: 'row', borderRadius: 20, marginBottom: 15, overflow: 'hidden' },
  floatingBadge: { position: 'absolute', top: 5, left: 5, height: 28, backgroundColor: '#D32F2F', borderRadius: 14, zIndex: 50, overflow: 'hidden' },
  badgeContent: { flexDirection: 'row', alignItems: 'center', height: 28, width: 210 },
  badgeIcon: { width: 28, textAlign: 'center' },
  floatingBadgeText: { color: '#FFF', fontSize: 11, fontWeight: 'bold', flex: 1 },
  imageWrapper: { width: 105, position: 'relative', backgroundColor: '#EEE', borderTopLeftRadius: 20, borderBottomLeftRadius: 20, overflow: 'hidden' },
  itemImage: { ...StyleSheet.absoluteFillObject, width: '100%', height: '100%' },
  detailsWrapper: { flex: 1, padding: 15, minHeight: 120 },
  ...
  itemDetails: { flex: 1, marginTop: 5 },
  itemName: { fontSize: 16, fontWeight: 'bold', flex: 1, paddingRight: 10 },
  itemContents: { fontSize: 13, marginBottom: 4 },
  expandedMissingBox: { marginTop: 6, marginBottom: 8, backgroundColor: '#FFEBEE', paddingVertical: 6, paddingHorizontal: 10, borderRadius: 8 },
  missingItemText: { color: '#D32F2F', fontSize: 12, fontWeight: 'bold', textDecorationLine: 'line-through', marginBottom: 2 },
  priceAndActionRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 'auto' },
  itemPrice: { fontSize: 16, fontWeight: 'bold' },
  rightActions: { flexDirection: 'row', alignItems: 'center' },
  editBtn: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12, marginRight: 10 },
  ...
  stickyFooter: { position: 'absolute', bottom: 0, left: 0, right: 0, borderTopLeftRadius: 30, borderTopRightRadius: 30, borderTopWidth: 1 },
  ...
  checkoutBtn: { backgroundColor: Colors.primary, paddingVertical: 18, borderRadius: 20, marginTop: 15, alignItems: 'center' },
  ...
  browseBtn: { backgroundColor: Colors.primary, paddingVertical: 16, paddingHorizontal: 35, borderRadius: 30 },
